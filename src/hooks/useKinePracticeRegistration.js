...
function validateSubmit(form, mode) {
  ...
  if (mode === 'new_kine') {
    if (!String(form.kineFirstname ?? '').trim() || !String(form.kineLastname ?? '').trim()) {
      return 'Vul je voor- en achternaam in.'
    }
    if (!String(form.kineEmail ?? '').trim()) return 'Vul je e-mailadres in.'
    if (String(form.kinePassword ?? '').length < MIN_PASSWORD) {
  ...
  if (form.plan === 'pro' && !form.invoice_same_as_practice && !String(form.invoice_name ?? '').trim()) {
...
export function useKinePracticeRegistration() {
  ...
  const submit = useCallback(async (form, mode, authUserId) => {
